...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($mdp)) {
        ...
        // On cherche l'utilisateur (on vérifie qu'il est actif)
        $stmt = $pdo->prepare("SELECT * FROM utilisateurs WHERE email = ? AND actif = 1");
        $stmt->execute([$email]);
        $user = $stmt->fetch();

        // On gère les deux noms de colonnes possibles pour le mot de passe par sécurité
        ...
        if ($user && password_verify($mdp, $hash)) {
            ...
            // ÉTAPE CRUCIALE POUR LE MULTI-ENSEIGNE
            // Si l'enseigne_id n'existe pas, on met 1 par défaut pour éviter de casser le dashboard
            $_SESSION['enseigne_id'] = $user['enseigne_id'] ?? 1;

            // On récupère les infos de l'enseigne pour l'affichage (Nom, Logo)
            $stmtE = $pdo->prepare("SELECT * FROM enseignes WHERE id = ?");
            $stmtE->execute([$_SESSION['enseigne_id']]);
            $ens = $stmtE->fetch();
            ...
            // On régénère l'ID de session pour la sécurité
...
<head>
  ...
  <style>
    /* Tes styles restent identiques, ils sont superbes */
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    input  { width:100%; padding:14px; border-radius:12px; background:#0d1117; border:1px solid rgba(255,255,255,.1); color:#fff; font-size:15px; outline:none; transition:border-color .2s; }
    ...
    button { width:100%; padding:16px; border-radius:12px; border:none; background:var(--red); color:#fff; font-size:16px; font-weight:800; cursor:pointer; transition:all .2s; margin-top:4px; }
